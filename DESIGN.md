...
タイトル直下に記事全体を要約するリード文（例：「〜の流れを追い、〜構造をひも解きます」）は**書かない**。
構成は「タイトル → KPISection → ArticleText（本文）」とする。
理由：KPISectionがすでに発見となる数字を提示しており、本文でその意味を説明しているため、
リード文は同じ内容の重複説明になりやすい。
